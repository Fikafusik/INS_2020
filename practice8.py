...
class MyCallback(keras.callbacks.Callback):
    ...
    def __init__(self, epochs, interval, train_target):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        self.batch_number = 0
        self.batch_accuracy = 100500.0
        self.batch_loss = 0
        self.batch_class = 0

    def on_epoch_end(self, epoch, logs=None):
        if (epoch + 1) % self.interval == 0 or epoch == 0 or (epoch + 1) == self.epochs:
            epoch_data = [(epoch + 1), self.batch_number, self.batch_accuracy, self.batch_loss,
                          self.batch_class]
            self.backs.append(epoch_data)

    def on_batch_end(self, batch, logs=None):
        accuracy = logs.get('acc')
        loss = logs.get('loss')
        if self.batch_accuracy > float(accuracy):
            self.batch_number = batch
            self.batch_accuracy = accuracy
            self.batch_loss = loss
            self.batch_class = self.train_target[batch]
...
train_target_label_not_encoded = train_target
...
myCallback = MyCallback(epochs, 2, train_target_label_not_encoded)

fit = model.fit(train_data, train_target, batch_size=10, epochs=epochs, verbose=1,
                validation_data=(test_data, test_target), callbacks=[myCallback])

table = np.asarray(myCallback.backs)
pd.DataFrame(np.asarray(myCallback.backs), columns=['epoch', 'batch_number', 'accuracy', 'loss', 'class']).to_csv('practice8.csv', index=False)
